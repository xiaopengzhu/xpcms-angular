...
(function($) {
    ...
    var methods = {
        jPlayer: function(options) {
            ...
            if(!$.support.leadingWhitespace) {
                var ie_dom = document.createElement("object");
                ie_dom.setAttribute("id", config.fid);
                ie_dom.setAttribute("classid", "clsid:d27cdb6e-ae6d-11cf-96b8-444553540000");
                ie_dom.setAttribute("codebase", "http://fpdownload.macromedia.com/pub/shockwave/cabs/flash/swflash.cab");
                ie_dom.setAttribute("type", "application/x-shockwave-flash");
                ie_dom.setAttribute("width", 0);
                ie_dom.setAttribute("height", 0);
                
                var obj_param = new Array();
                obj_param[0] = {name:"movie", value:config.swf};
                obj_param[1] = {name:"quality", value:"high"};
                obj_param[2] = {name:"FlashVars", value:"id=' + escape(config.id) + '&fid=' + escape(config.fid) + '"};
                obj_param[3] = {name:"allowScriptAccess", value:"always"};
                obj_param[4] = {name:"bgcolor",value:config.bgcolor};
                
                for(var i=0; i < obj_param.length; i++) {
                    var param = document.createElement("param");
                    param.setAttribute("name", obj_param[i].name);
                    param.setAttribute("value", obj_param[i].value);
                    ie_dom.appendChild(param);
                }
                $(this).html(ie_dom);
            } else {
...
$(function(){
    $("#jquery_jplayer").jPlayer({
        swfPath:ROOT+"/Public/plugins/jplayer"            
    });
    ...
    function updateLyrics(time){
        var dom = $("#lrc_content li[time='"+time+"']");

        if (dom.length > 0) {
            if (!dom.hasClass("high")) {
                $("#lrc_content li").removeClass("high");
                dom.addClass("high");
                var off = parseInt(dom.css("height"));
                lrctop = lrctop  - off;
                $("#lrc_content").animate({top: lrctop+"px"});
            }
        }
    }
    ...
    function loadLyrics(id) {
        ...
        $.ajax({
            type: "GET",
            url: URL+"/music/getLyrics/id/"+id,
            dataType: "json",
            success: function(data){
                var html='';
                $.each(data, function(i,item){
                    html+="<li time='"+item.time+"'>&nbsp;"+item.value+"</li>"
                });
                $("#lrc_content").html(html).css({top:lrctop+"px"});
            },
            error: function(){
                $("#lrc_content").html("").css({top:lrctop+"px"});
            }
        }); 
    }
